            ...
            {stream.map((char, index) => (
                <a
                    style={{
                        ...
                        color: index === stream.length - 1 ? '#484B60' : undefined,
                        textShadow:
...
const MatrixRain = (props) => {
    ...
    // @ts-ignore
    const streamCount = containerSize ? Math.floor(width / 29) : 0;
    ...
    return (
        <div
            ...
            {// @ts-ignore
                new Array(streamCount).fill().map(_ => (<RainStream height={containerSize?.height}/>
                ))}
        </div>
    );
};
